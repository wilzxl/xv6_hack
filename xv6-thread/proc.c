...
//The is the join syscall waiting for thread,return pid
int 
join(int threadId, void** stack)
{
  ...
  for(;;){
    ...
    // wait for children same as wakeup1()
    sleep(proc, &ptable.lock);
  }
}

int threadSleep(newpthread_mutex_t *mutex){
  if(proc==0)
    panic("sleep");
